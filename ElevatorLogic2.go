...
func Externalrequest(localip string, Elevatorlist []extra.Elevator) Network.Request {
...
var statelist = make(map[string]Network.Info)
    ...
    Internalloop:
    for _, requests := range internalrequestlist {
...
func Nextrequest(localip string, Elevatorlist []extra.Elevator) Network.Request {
    ...
    for host, info := range infolist {
     ...
     go Externalrequest(localip, Elevatorlist)
     go Internalrequest(localip, Elevatorlist)
